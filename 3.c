...
scanf("%d",&n)
printf("enter producive grammer");
 for(i=0;i<n;i++)
scanf("%s",st[i]); 
for(i=0;i<n;i++) 
fol[i][0]='\0';
...
printf("first pos"); for(i=0;i<n;i++)
printf("first[%c]=%s\n",st[i][0],ft[i]); fol[0][0]='$';
for(i=0;i<n;i++)
{
k=0; j=3;
...
printf("follow pos"); for(i=0;i<n;i++)
printf("follow[%c]=%s",st[i][0],fol[i]); printf("\n");
s=0;
for(i=0;i<n;i++)
{ j=3;
while(st[i][j]!='\0')
